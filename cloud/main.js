...
const promisifyW = pp => pp.catch(error => error);
...
const setTableData = (table, data, method = 'POST') => {
  ...
  .then(response => {
    ...
    throw 'Error setting table data';
  });
};
...
Parse.Cloud.define("deleteContentItem", (request) => {
  if (!request.user) {
    return 'Must be signed in to call this Cloud Function.';
  }
  ...
  return deleteContentItem(request.user, tableName, itemId)
    ...
    .catch(error => { throw ("Could not delete content item: " + JSON.stringify(error, null, 2)); });
});
...
Parse.Cloud.define("deleteModel", (request) => {
  ...
  return new Parse.Query("Model")
    ...
    .catch(error => { throw ("Could not delete model: " + JSON.stringify(error, null, 2)); });
});
...
Parse.Cloud.define("deleteSite", (request) => {
  ...
  return new Parse.Query("Site")
    ...
    .catch(error => { throw ("Could not delete site: " + JSON.stringify(error, null, 2)); });
});
...
Parse.Cloud.define("onContentModify", (request) => {
  ...
  if (!URL) {
    throw 'Warning! There is no content hook!';
  }
...
Parse.Cloud.define("inviteUser", function(request) {
  if (!request.user) {
    return 'You must be authorized!';
  }
